...
def planner_with_split(planner, observation, hidden_states, cell_states ,model_params):
    # # we have to consider batch size here;
    # # no batch
    ...
    #     # the lists passed to the planner: (num_levels, 2 (layers), hidden_size_at_level)
    ...
    #     # second rows: shape (2, start_hidden_size-difference) note dim=1 because the returned results are lists
    ...
    # # hidden states are in batch. We pass them to the model still in lists, but with an additional batch dimension
    ...
    new_hidden_states_second_row = torch.cat((new_hidden_states_second_row, torch.zeros(new_hidden_states_second_row.shape[0], 2, difference)), dim=2)
    new_cell_states_second_row = torch.cat((new_cell_states_second_row, torch.zeros(new_cell_states_second_row.shape[0], 2, difference)), dim=2)
...
value_module = ValueNetwork(task_params=EnvConstants.TASK_PARAMS['pushing'])
